...
char productions[20][SIZE] = {};// to save prod after lr remove 
int prodCount = 0;// count
...
int main() {
    ...
    char nont[20] = {};
    for(int i=0; i<prodCount; i++) { // disnict non terminals
        printf("%s\n", productions[i]);
        int f = 1;
    ...
    while(i < prodCount) {
        for(int j=2; j<strlen(productions[i]); j++) {
            if(productions[i][j] == ch && productions[i][0] != ch) {
                // printf("yes");
                if(strlen(productions[i]) - 1 == j) { // last anu
