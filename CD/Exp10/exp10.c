...
char DFA_finals[10];   /* final-state string */
    ...
    char *finals)
{
    ...
    /* input symbols: '0', '1', ... */
    printf("     | ");
    for (i = 0; i < nsymbols; i++) printf("  %c  ", 'a'+i);
...
void load_DFA_table()
{
	...
	scanf("%i",&N_DFA_states);
	printf("Enter the number of symbols\n");
	scanf("%i",&N_symbols);
	printf("Enter transition table\n");
	for(int yy=0;yy<N_DFA_states;yy++){
		for(int zz=0;zz<N_symbols;zz++){
			//scanf("%c",&DFAtab[yy][zz]);
			char vv,vvv[10];
			scanf("%s",vvv);
			vv=vvv[0];
			DFAtab[yy][zz]=(int)vv;
			//printf("cc %i, YY=%i ZZ=%i\n",DFAtab[yy][zz],yy,zz);
		}
	}
